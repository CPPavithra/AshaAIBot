...
useEffect(() => {
  if (email) {
      fetch(`https://ashaaibot-server.onrender.com/chat-history?email=${encodeURIComponent(email)}`)
        .then(res => {
          if (!res.ok) {
 ...
 // Scroll to the latest message
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch("https://ashaaibot-server.onrender.com/ai-response", {
        method: "POST",
